...
files that match the backtester schema exactly.
...
# Parquet column schema — matches backtester snapshot_builder.py exactly
_OPTION_COLS = [
...
# 1000 instruments x 300 data/day = 300K rows.
...
class Snapshotter:
    ...
    def __init__(self):
        ...
        # Deribit vs server clock offset — kept current from every incoming tick.
        # deribit_time = server_time + _deribit_offset_us (microseconds).
        self._deribit_offset_us = 0  # type: int

        os.makedirs(config.DATA_DIR, exist_ok=True)

    # ── External API ─────────────────────────────────────────────────────────

    def update_deribit_offset(self, deribit_us):
        # type: (int) -> None
        """Update Deribit clock offset from a received tick timestamp (microseconds).

        Clamps to ±30 seconds to reject clearly stale or bogus values.
        Re-anchors the schedule to Deribit exchange time on every call.
        """
        offset = deribit_us - _now_us()
        if -30_000_000 <= offset <= 30_000_000:
            self._deribit_offset_us = offset

    def deribit_now_us(self):
        # type: () -> int
        """Current Deribit exchange time in microseconds (server clock + measured offset)."""
        return _now_us() + self._deribit_offset_us

    @property
    def deribit_offset_secs(self):
        # type: () -> float
        """Measured offset of Deribit exchange clock vs server clock, in seconds."""
        return self._deribit_offset_us / 1_000_000
    ...
    def on_tick(self, channel, data):
        ...
        # Keep Deribit clock offset fresh from every incoming tick.
        ts_ms = data.get("timestamp")
        if ts_ms is not None:
            try:
                self.update_deribit_offset(int(ts_ms) * 1000)
    ...
    def _handle_spot_tick(self, data):
        ...
        # Use Deribit's own timestamp for bar alignment so bars match exchange
        # time exactly regardless of server clock accuracy.
        ts_ms = data.get("timestamp")
        try:
            now_us = int(ts_ms) * 1000 if ts_ms is not None else _now_us()
        except (TypeError, ValueError):
            now_us = _now_us()
